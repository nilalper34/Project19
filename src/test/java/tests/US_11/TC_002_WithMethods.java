...
public class TC_002_WithMethods extends TestBaseRapor {
    ...
    public void testCase002() throws InterruptedException {
        extentTest=extentReports.createTest("Pozitif Test","Units per piece");
        ...
        //"Units Per Piece" alanının erişilebilir olduğu test edilir ve geçerli bir sayı girilir
        wait.until(ExpectedConditions.visibilityOf(hub.inputUnitPerPiece));
        hub.inputUnitPerPiece.isEnabled();
        hub.inputUnitPerPiece.clear();
        hub.inputUnitPerPiece.sendKeys(""+random.nextInt(100));
        extentTest.info("\"Units Per Piece\" alanının erişilebilir olduğu test edilir ve geçerli bir sayı girildi");
